#!/usr/bin/env python3
"""batch_deepseek_repeat.py — Run repeat lattice for Test 1 stability."""
import json, os, sys, time, requests
from datetime import datetime, timezone
from concurrent.futures import ThreadPoolExecutor, as_completed

OUTPUT_DIR = "/home/z/my-project/download/lcge_engine/lattice/output"
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "runs_deepseek_repeat.jsonl")
...
def make_record(lp, rep, run_id, content, fr):
    return {
...
def main():
    ...
    for lp in lattice:
        for rep in range(NUM_REPS):
            rid = f"{lp['run_key']}_r{rep:003d}"
            if rid not in done_keys:
                pending.append((lp, rep, rid))
    ...
    print(f"Total: {total}, Done: {len(done_keys)}, Pending: {len(pending)}", flush=True)
    ...
    with open(OUTPUT_FILE, "a") as out, ThreadPoolExecutor(max_workers=MAX_WORKERS) as pool:
        ...
        for future in as_completed(futures):
            ...
            if fr == "error":
                errors += 1
            rec = make_record(lp, rep, rid, content, fr)
    ...
    elapsed = time.time() - start
    print(f"DONE: {len(done_keys)}/{total} in {elapsed:.0f}s, errors={errors}", flush=True)
